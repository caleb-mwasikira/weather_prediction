...
# get_recommendations function (modified for wider data window)
def get_recommendations(weather_data, my_plant):
    ...
    # Ensure weather_data is a DataFrame for consistent access
    ...
    # Calculate averages over the provided period (which is now the wider rolling window)
    ...
    # Get the most recent precipitation value to check if it's "currently raining"
    # This assumes the weather_data DataFrame is sorted chronologically by index.
    recent_precip = weather_data['precip'].iloc[-1] if not weather_data['precip'].empty else 0
    
    # Define what constitutes "currently raining" (e.g., more than 0.5mm in the last reading)
    is_currently_raining = recent_precip > 0.5

    # Planting Conditions Recommendations
    # Prioritize "too much rainfall" based on the average over the wider period
...
# Define the rolling window size in days (e.g., 21 days for 3 weeks)
...
def get_weekly_recommendations(month, day):
    ...
    try:
        # Use the year of the first data point in the 'weather' DataFrame for consistency
        # This assumes your weather data covers the requested month/day.
        year = weather.index[0].year
        query_date = datetime(year, month, day)
    except ValueError:
        return jsonify({"error": "Invalid date."}), 400

    # Determine the end date of the week for the query (Saturday of the given week)
    # This aligns the rolling window's end with the natural week boundary for consistency.
    ...
    # Ensure date comparisons are done on the date part only to avoid time component issues
    ...
    # Pass the filtered rolling weather data to the recommendation function
